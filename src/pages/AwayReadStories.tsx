...
function AwayReadStories({
  ...
  const { owner, post, time, _id, url } = route.params;
  ...
  useEffect(() => {
    const foo = async () => {
      await axios.get(`mystorie/${owner}`).then((response) => {
        if (response.data === "no") {
          Alert.alert(
            `@${owner} no ha subido historias`,
            "Sera dirigido hacia el Menu Principal",
...
export default AwayReadStories;
